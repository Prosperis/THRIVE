    ...
    <div className="flex items-center justify-center gap-1">
      ...
      <Select value={currentYear.toString()} onValueChange={handleYearChange}>
        <SelectTrigger className="h-6 w-auto gap-1 border-0 px-1 text-xs font-medium hover:bg-accent focus:ring-0">
          <SelectValue>
            {isPersian ? toPersianNumber(currentYear) : currentYear}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
          {years.map((year) => (
...
function CalendarChevron({ 
  ...
  if (orientation === "left") {
    return (
      <ChevronLeftIcon className={cn("h-3.5 w-3.5", className)} {...props} />
    )
  }
  ...
  return (
    <ChevronDownIcon className={cn("h-3.5 w-3.5", className)} {...props} />
  )
}
